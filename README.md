...
## The parts that aren't just a paper implementation
...
- **A negative result taken seriously and redesigned, not hidden.** The first version of this mechanism promoted hot nodes to higher graph layers, the standard "make this node more prominent" instinct in a hierarchical graph. Measured against a deep-copied static baseline on an identical, fixed query set, that turned out not to help at all. See [the diagnosis](#promotion-the-idea-that-didnt-survive-contact-with-data) below for why, and what replaced it.
...
## Promotion: the idea that didn't survive contact with data
...
## What self-tuning actually costs
...
## Regenerating every chart above

```bash
python -m adaptann.demos.benchmark            # the drifting-workload charts and the degree-widening chart
python -m adaptann.demos.promotion_diagnostic # the abandoned-promotion-mechanism diagnostic chart
